...
.block 作为函数返回值返回时，编译器自动将 block 作为 \_Block\_copy 函数，效果等同于 block 直接调用 copy 方法；


.block 被赋值给 \_\_strong id 类型的对象或 block 的成员变量时，编译器自动将 block 作为 _Block_copy 函数，效果等同于 block 直接调用 copy 方法；

.。block 作为参数被传入方法名带有 usingBlock 的 Cocoa Framework 方法或 GCD 的 API 时。这些方法会在内部对传递进来的 block 调用 copy 或 \_Block\_copy 进行拷贝;
